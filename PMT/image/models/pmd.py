...
class PlainMaskDecoder(nn.Module):
    ...
    def _predict(self, x: torch.Tensor):
        ...
        quality_logits = self.quality_head(q) if self.quality_head_enabled else None
        ...
        if self.quality_head_enabled:
            return mask_logits, class_logits, quality_logits
    ...
    def forward(
        ...
        quality_logits_per_layer = [] if self.quality_head_enabled else None

        for i, block in enumerate(self.blocks):
            if self.masked_attn_enabled:
                prediction = self._predict(self.decoder_norm(x))
                if self.quality_head_enabled:
                    mask_logits, class_logits, quality_logits = prediction
                    quality_logits_per_layer.append(quality_logits)
                else:
                    mask_logits, class_logits = prediction
        ...
        if self.quality_head_enabled:
            return mask_logits_per_layer, class_logits_per_layer, quality_logits_per_layer
