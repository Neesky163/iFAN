...
CONFIG_PATH="${CONFIG_PATH:-${CHECKPOINT_ROOT}/instance/configs/coco_instance_L_640.yaml}"
CKPT_PATH="${CKPT_PATH:-${CHECKPOINT_ROOT}/instance/inference/coco_instance_L_640.ckpt}"
DATA_PATH="${DATA_PATH:-}"
GPU="${GPU:-0}"
LOG_ROOT="${LOG_ROOT:-${EOMT_ROOT}/metric_test_results}"

[[ -n "${DATA_PATH}" ]] || { echo "DATA_PATH is required (COCO 2017 validation root)" >&2; exit 2; }
