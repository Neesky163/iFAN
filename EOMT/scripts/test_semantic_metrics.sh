...
CONFIG_PATH="${CONFIG_PATH:-${CHECKPOINT_ROOT}/semantic/configs/ade20k_semantic_L_512.yaml}"
CKPT_PATH="${CKPT_PATH:-${CHECKPOINT_ROOT}/semantic/inference/ade20k_semantic_L_512.ckpt}"
DATA_PATH="${DATA_PATH:-}"
GPU="${GPU:-0}"
LOG_ROOT="${LOG_ROOT:-${EOMT_ROOT}/metric_test_results}"

[[ -n "${DATA_PATH}" ]] || { echo "DATA_PATH is required (ADE20K or Cityscapes validation root)" >&2; exit 2; }
